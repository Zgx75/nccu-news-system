...
def generate_email_html_content(df, observation_items):
    ...
    def section_bar(title, bg="#dbe7f3"):
        return f'<div style="background:{bg}; padding:10px 12px; margin-top:24px; font-weight:bold;">{title}</div>'
    ...
    def news_list_html(rows):
        ...
        for _, r in rows.iterrows():
            ...
            <li style="margin-bottom:8px;">
                <a href="{r['連結']}" style="color:#0057c2; text-decoration:underline;">{r['標題']}</a>
                <span>（{r['媒體']}）</span>
            </li>"""
        html += "</ul>"
        return html
    ...
    <div style="background:#dbe7f3; padding:24px 30px; margin-bottom:42px;">
        <h1 style="font-size:30px; margin:0; font-weight:bold;">{today_short} 政大輿情彙整</h1>
    </div>
    <p>各位師長、同仁好：</p>
    <p>以下為 <strong>{today_full}</strong> 新聞輿情，敬請參閱。<br>如對相關議題有興趣，請直接點選標題連結。</p>
    <div style="border:1px solid #d6e0ea; background:#f8fbff; padding:18px 22px; margin:26px 0;">
    ...
    for _, r in df.iterrows():
        ...
        <tr>
            <td>{r['類別']}</td>
            <td>{r['主題']}</td>
            <td><a href="{r['連結']}" style="color:#0057c2; text-decoration:underline;">{r['標題']}</a></td>
            <td>{r['媒體']}</td>
        </tr>"""
    ...
    English｜<a href="https://youtu.be/1ViR0M-UB3o">https://youtu.be/1ViR0M-UB3o</a></p>
...
st.set_page_config(page_title="政大新聞輿情自動化系統", layout="wide", page_icon="📡")

st.title("📡 政大新聞輿情自動化搜集系統")
st.caption("透過 Google News RSS 與 Gemini API 進行多通路自動化即時輿情清洗與摘要")
...
days_to_fetch = st.sidebar.slider(
    ...
    max_value=14,
    value=1,
    help="設定往前追蹤的新聞發布時間 window。例如：三天連假後可以選擇 3 或 4。"
)

max_results_per_kw = st.sidebar.number_input(
    "3. 每組關鍵字最大抓取量",
    min_value=10,
    max_value=100,
    value=50,
...
# 開始按鈕
if st.sidebar.button("🚀 開始執行輿情爬取任務", use_container_width=True):
    with st.status("🛸 輿情系統執行中，請稍候...", expanded=True) as status:
        ...
        st.info(f"📅 目前時間設定追蹤：過去 {days_to_fetch} 天內的新聞 ({time_param})")
        ...
        for kw in keyword_list:
            ...
            valid_entries = feed.entries[:max_results_per_kw]
        ...
        if news_list:
            ...
            observation_items = ai_generate_bullets("輿情觀察", df, [
                "政大相關新聞以校務與活動曝光為主。",
                "高教議題聚焦 AI、校園建設與教學爭議。",
                "整體新聞風向偏中性穩定。"
            ])
            ...
            status.update(label="✅ 輿情搜集與分析任務全面成功！", state="complete", expanded=False)
        else:
            st.session_state.df_result = pd.DataFrame()
            status.update(label="📭 任務完成，但所選時間區間內查無任何相關新聞動態。", state="complete", expanded=True)
...
if st.session_state.df_result is not None:
    if st.session_state.df_result.empty:
        st.warning("📭 您所選擇的日期區間內，未篩選到任何符合高相關度門檻的新聞動態。建議調整回溯天數或關鍵字重試。")
    else:
        st.success("🎉 數據已成功加載！您可以在下方預覽或直接下載匯出報表。")
        ...
        metric_col1.metric("總收錄新聞篇數", f"{len(st.session_state.df_result)} 篇")
        metric_col2.metric("政大陣營新聞", f"{len(st.session_state.df_result[st.session_state.df_result['類別']=='政大'])} 篇")
        metric_col3.metric("高教大環境新聞", f"{len(st.session_state.df_result[st.session_state.df_result['類別']=='高教'])} 篇")

        st.subheader("🔍 互動式數據篩選與檢視面板")
        
        selected_cat = st.multiselect(
            "過濾顯示類別", 
            options=list(st.session_state.df_result["類別"].unique()),
            default=list(st.session_state.df_result["類別"].unique())
        )
        
        filtered_df = st.session_state.df_result[st.session_state.df_result["類別"].isin(selected_cat)]
        
        st.dataframe(
            filtered_df[["發布時間", "類別", "主題", "媒體", "標題", "摘要", "連結"]],
            column_config={
                "連結": st.column_config.LinkColumn("新聞外部連結")
            },
            hide_index=True,
            use_container_width=True
        )
else:
    st.info("💡 請於左側設定您所需的「關鍵字」與「連假追蹤回溯天數」，並點擊『開始執行輿情爬取任務』按鈕。系統將自動為您生成今日簡報。")